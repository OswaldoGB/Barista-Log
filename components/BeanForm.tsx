...
import { CoffeeBean, ProcessType, RoastLevel } from '../types';
import { EMPTY_BEAN, PROCESSES, ROAST_LEVELS, TRANSLATIONS, LABELS } from '../constants';
...
const BeanForm: React.FC<Props> = ({ initialData, onSave, onCancel, lang }) => {
    ...
    <form onSubmit={(e) => { e.preventDefault(); onSave(formData); }} className="max-w-2xl mx-auto bg-white dark:bg-stone-900 p-6 rounded-2xl shadow-sm border border-stone-200 dark:border-stone-800 w-full">
      <div className="flex justify-between items-center mb-6 border-b border-stone-100 dark:border-stone-800 pb-4">
        <h2 className="text-xl font-bold text-stone-900 dark:text-stone-100 flex items-center gap-2">
          ...
          {initialData ? t.editCoffee : t.newCoffee}
        </h2>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="label">{t.farmProducer}</label>
            <input type="text" className="input" 
              value={formData.farm || ''} onChange={e => handleChange('farm', e.target.value)} />
          </div>
          <div>
            <label className="label">{t.altitude}</label>
            <input type="text" className="input" 
          ...
          <div>
            <label className="label">{t.process}</label>
            <select className="input" value={formData.process} onChange={e => handleChange('process', e.target.value)}>
              {PROCESSES.map(p => (
                <option key={p} value={p}>{LABELS.process[p as keyof typeof LABELS.process]?.[lang] || p}</option>
              ))}
            </select>
          </div>
        </div>
